...
object Effects {
  ...
  // 2
  def measure[A](computation: MyIO[A]): MyIO[Long] = clock
    .map(initialTime => {
      computation.unsafeRun()
      clock.unsafeRun() - initialTime
    })
  def measure_v2[A](computation: MyIO[A]): MyIO[Long] = for {
    startTime <- clock
    _ <- computation
    finishTime <- clock
  ...
  def main(args: Array[String]): Unit = {
    ...
    val measureComputation = measure[Unit](MyIO(() => TimeUnit.MILLISECONDS.sleep(4521)))
    println(measureComputation.unsafeRun())
