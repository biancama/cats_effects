...
object CatsTypeClasses {
  ...
  val optionList: Option[List[Int]] = listTraverse.traverse(List(1, 2, 3))(x => Option(x))  //Some(List(1, 2, 3))

  import cats.syntax.traverse.*
  val optionList_v2: Option[List[Int]] = List(1, 2, 3).traverse(x => Option(x)) //Some(List(1, 2, 3))
  val listOfOption_v2 = List(1, 2, 3).map(x => Option(x)) //List(Some(1), Some(2), Some(3))
